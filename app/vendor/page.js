...
import { Building2, Phone, Mail, Briefcase, MapPin, FileText, CheckCircle2, Handshake } from 'lucide-react';

const TRADE_CATEGORIES = [
  'Electrical (HT/LT Works)',
  'Civil & Structural',
  'Mechanical & HVAC',
  'IT & Networking',
      ...
      <section className="gradient-navy text-white relative overflow-hidden py-20 pt-36">
        ...
        <div className="relative max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="mt-4 text-base text-white/70 max-w-2xl">
            IndusVertex is always looking for reliable subcontractors, suppliers and service partners across electrical, civil, mechanical, IT and environmental disciplines. Register your firm and our procurement team will reach out.
          </p>
        </div>
      </section>
          ...
          {submitted ? (
            ...
            <div className="bg-white dark:bg-[#0a1628] rounded-2xl shadow-xl p-8 border border-slate-100 dark:border-white/10">
              <div className="mb-6">
                ...
                <p className="text-slate-500 text-xs mt-1">Fill in your details and our team will evaluate your profile for empanelment.</p>
              </div>
              ...
              <form onSubmit={submit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input required value={form.companyName} onChange={e => setForm({ ...form, companyName: e.target.value })} className="mt-1 h-9 text-sm" placeholder="Your registered firm name" />
                  </div>
                  <div>
                    <Label className="text-xs font-semibold text-slate-600">Contact Person *</Label>
                    <Input required value={form.contactPerson} onChange={e => setForm({ ...form, contactPerson: e.target.value })} className="mt-1 h-9 text-sm" placeholder="Owner / Director name" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input required value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} className="mt-1 h-9 text-sm" placeholder="+91 XXXXX XXXXX" />
                  </div>
                  <div>
                    <Label className="text-xs font-semibold text-slate-600">Email *</Label>
                    <Input required type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="mt-1 h-9 text-sm" placeholder="company@email.com" />
                  </div>
                </div>

                <div>
                  <Label className="text-xs font-semibold text-slate-600">Trade / Service Category *</Label>
                  <select required value={form.tradeCategory} onChange={e => setForm({ ...form, tradeCategory: e.target.value })}
                    className="mt-1 w-full h-9 px-3 text-sm rounded-md border border-input bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-ring">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input value={form.gst} onChange={e => setForm({ ...form, gst: e.target.value })} className="mt-1 h-9 text-sm" placeholder="15-digit GSTIN" />
                  </div>
                  <div>
                    <Label className="text-xs font-semibold text-slate-600">Years of Experience</Label>
                    <Input value={form.experience} onChange={e => setForm({ ...form, experience: e.target.value })} className="mt-1 h-9 text-sm" placeholder="e.g. 8 years" />
                  </div>
                </div>

                <div>
                  <Label className="text-xs font-semibold text-slate-600">Areas of Operation</Label>
                  <Input value={form.areasOfOperation} onChange={e => setForm({ ...form, areasOfOperation: e.target.value })} className="mt-1 h-9 text-sm" placeholder="e.g. Delhi NCR, UP, Rajasthan" />
                </div>

                <div>
                  <Label className="text-xs font-semibold text-slate-600">Additional Details</Label>
                  <Textarea rows={3} value={form.message} onChange={e => setForm({ ...form, message: e.target.value })}
                    placeholder="Brief description of your firm's capabilities, past projects, certifications, etc."
